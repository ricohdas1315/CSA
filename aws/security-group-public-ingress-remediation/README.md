...
## Slightly better version with “manual test” path too

If you want the fuller version showing both manual testing and real automation, use this instead:
...
```mermaid
flowchart LR
    A1[Manual test event with group_id] --> D[AWS Lambda<br/>Auto-Remediation Function]
    A2[User adds or modifies inbound Security Group rule] --> B[AWS CloudTrail]
    B --> C[Amazon EventBridge Rule]
    C --> D

    D --> E[Extract Security Group ID]
    E --> F[Describe Security Group]

    F --> G{AllowPublicIngress=true tag?}
    G -- Yes --> H[Skip remediation]
    G -- No --> I[Inspect inbound rules for<br/>0.0.0.0/0 or ::/0]

    I --> J{Public inbound rule found?}
    J -- No --> K[Log result to CloudWatch]
    J -- Yes --> L[Revoke offending inbound rule]
    L --> K
